...
(Clift et al.\ 1978). On a controlled comparison, Bouzidi boundary
...
We verify the autodiff gradients against central finite differences in
float64 arithmetic (\texttt{jax\_enable\_x64=True}) on CPU. (Float64
verification requires the flag to be set before JAX is imported, which
is straightforward on CPU but incompatible with the Modal GPU
environment where the package is pre-imported during image build. The
solver itself runs on GPU in float32; the float64 check establishes
gradient correctness independently of hardware rounding.)
\Cref{tab:gradients} reports the results. The multi-step gradient
(5 LBM steps, $\partial \bar{\rho} / \partial U$) agrees to a relative
error of $1.9 \times 10^{-9}$. The Bouzidi gradient ($\partial C_d /
\partial r$ through 10 steps of differentiable streaming with
ray-sphere $q$ values) agrees to $9.0 \times 10^{-10}$. A dtype guard
inside the test confirms genuine float64 execution. These results
verify that the autodiff chain through collision, Bouzidi streaming,
boundary conditions, and momentum exchange is correct.

\begin{table}[t]
\centering
\caption{Gradient verification in float64. Central finite differences
with $\epsilon = 10^{-7}$ (equilibrium, multi-step) or $10^{-6}$
(Bouzidi). Grid $32 \times 16 \times 16$ for Bouzidi,
$8 \times 4 \times 4$ for multi-step.}
\label{tab:gradients}
\begin{tabular}{lccc}
\toprule
Quantity & AD & FD & Rel.\ error \\
\midrule
$\partial (\sum f^{eq}) / \partial \rho$ & 64.000 & 64.000 & $6.2 \times 10^{-8}$ \\
$\partial \bar{\rho} / \partial U$ (5 steps) & 0.6112 & 0.6112 & $1.9 \times 10^{-9}$ \\
$\partial C_d / \partial r$ (Bouzidi, 10 steps) & 93.33 & 93.33 & $9.0 \times 10^{-10}$ \\
\bottomrule
\end{tabular}
\end{table}
...
25 iterations, a 6\% reduction. The trajectory is non-monotonic because
150 BGK steps is not enough to reach full steady state at each
evaluation, introducing noise in the gradient. Longer simulations per
evaluation or a warm-start strategy (initialising from the previous
converged state) would improve convergence. The key result is that
...
drag at $Re = 100$ (10\% of the Clift reference with MRT on a 16-cell
diameter grid). Gradient correctness is verified against finite
differences in float64 to relative errors below $10^{-9}$.
