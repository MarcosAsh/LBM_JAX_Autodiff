...
(the neighbor at $\mathbf{x} + \mathbf{e}_i$), which widens the
...
error is larger than typical for LBM at this resolution; Mei et
al.~\cite{mei2002} report errors under 5\% at comparable diameters with
carefully tuned MRT parameters. Our higher error is likely a combination
of the modest blockage ratio ($D/L_y = 0.25$), MRT damping parameter
choices, and the relatively short run (4000 steps may not be fully
converged at this Re). To confirm that the error is dominated by grid
resolution rather than a solver defect, we provide a fine-grid script
(\texttt{09\_sphere\_cd\_fine\_grid.py}) that doubles the resolution to
$D = 32$ on a $256 \times 128 \times 128$ grid. At this resolution, $C_d$
approaches the Clift reference to within 5\%, consistent with the
expected first-to-second-order convergence of the Bouzidi boundary
scheme. A systematic grid convergence study across three or more
resolutions is left to future work.

The BGK operator on the coarse grid produces $C_d = 0.75$ with large
...
Planned extensions include differentiable ray-triangle intersection for
mesh-based geometries (removing the restriction to analytic shapes), a
systematic grid convergence study of the gradient across three or more
resolutions, and application to more complex bodies such as Ahmed body
drag reduction at higher Reynolds numbers.
