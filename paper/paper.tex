...
\Cref{tab:bouzidi_comparison} summarises the results over 25 Adam
iterations on a $48 \times 24 \times 24$ grid with 150 BGK steps per
evaluation.

\begin{table}[h]
\centering
\caption{Bouzidi vs standard bounce-back: optimisation comparison on A100.
Same grid, physics, and optimiser. Only the wall treatment differs.}
...
Average $|\partial C_d / \partial r|$ & 7.74 & 0.22 \\
...
than the standard bounce-back gradients. This is because the Bouzidi
$q$ values carry continuous information about the wall position within
each lattice cell, providing a rich gradient signal. Standard
bounce-back reduces this to a binary choice (wall at midpoint or not),
which nearly flattens the gradient landscape. The optimisation with
Bouzidi reaches a $C_d$ that is 37\% lower than the best standard
bounce-back result, demonstrating that second-order boundary treatment
produces qualitatively better shape gradients.

\subsection{Grid convergence of the gradient}

We compute $\partial C_d / \partial r$ at multiple grid resolutions
using the differentiable sphere geometry. On $32 \times 16 \times 16$
and $48 \times 24 \times 24$ grids, the autodiff gradient is finite,
nonzero, and agrees with central finite differences in sign. On larger
grids ($64^3$ and above), the memory cost of carrying two full
distribution arrays through \texttt{jax.lax.scan} exceeds A100 GPU
memory (40\,GB). This can be resolved with gradient checkpointing at
the scan level, which is supported by JAX but was not applied in these
experiments to keep the comparison fair.

The consistency of gradient sign across resolutions, combined with the
finite-difference verification at small grids, provides evidence that
the computed gradient is physically meaningful. A full convergence study
at higher resolutions remains future work pending memory optimisation.
