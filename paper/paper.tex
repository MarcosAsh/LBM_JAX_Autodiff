...
machinery. We verify directly that $\partial C_d / \partial r = 0$
...
gradient path; we report its benefits and its pitfalls (degenerate
...
path. The main comparison we draw is not with a weaker variant of
the Bouzidi pipeline: it is with classical LBM itself (halfway
bounce-back + binary solid mask), for which we show the shape
...
\Cref{sec:optim_soft}). We view these numbers as the realistic
...
48x24x24, 150-step setup we tried: that grid produced a non-physical
...
step, and the Bouzidi $q$ on the streaming step), we run the same
...
\begin{enumerate}\setlength\itemsep{0pt}
    ...
    which in our experiments is the hard-mask case where halfway
...
$\alpha$ grows and the transition localises. Our choice of $\alpha
...
We have presented a differentiable D3Q19 Lattice Boltzmann solver
...
is impossible in that setting. Our pipeline closes this gap:
...
second gradient path through the collision step; we find that this
