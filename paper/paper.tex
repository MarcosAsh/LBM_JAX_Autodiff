...
We verify gradients against central finite differences (step size
$\epsilon = 10^{-4}$) in float64 mode (\texttt{JAX\_ENABLE\_X64}).
\Cref{tab:gradients} reports results for three representative functions.

\begin{table}[t]
\centering
\caption{Gradient verification in float64.}
\label{tab:gradients}
\begin{tabular}{lccc}
\toprule
...
All three agree to better than $10^{-7}$ relative error, confirming that
reverse-mode AD correctly differentiates the equilibrium, streaming, and
Bouzidi boundary components of the solver. Verification was performed on
CPU; the GPU solver uses float32 for performance.
