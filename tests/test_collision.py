...
class TestMRTTransforms:
    ...
    def test_roundtrip_weights(self):
        ...
        assert jnp.allclose(f_cell, f_recovered, atol=5e-4)
    ...
    def test_conserved_moments_match_macroscopic(self):
        ...
        f_cell = f_eq[:, 0, 0, 0]
        m = mrt_forward(f_cell)
        assert jnp.allclose(m[0], rho_val, atol=5e-4)
        assert jnp.allclose(m[3], rho_val * u_val[0], atol=5e-4)
        assert jnp.allclose(m[5], rho_val * u_val[1], atol=5e-4)
        assert jnp.allclose(m[7], rho_val * u_val[2], atol=5e-4)
    ...
    def test_equilibrium_moments(self):
        ...
        assert jnp.allclose(m_from_f, m_eq, atol=5e-4)
...
class TestBGK:
    ...
    def test_equilibrium_fixed_point(self):
        ...
        assert jnp.allclose(f, f_post, atol=5e-4)
    ...
    def test_conserves_mass(self):
        ...
        assert jnp.allclose(rho_before, rho_after, atol=5e-4)
    ...
    def test_conserves_momentum(self):
        ...
        assert jnp.allclose(rho_before * u_before, rho_after * u_after, atol=5e-4)
...
class TestSmagorinsky:
    ...
    def test_at_equilibrium_tau_unchanged(self):
        ...
        assert jnp.allclose(tau_eff, 0.8, atol=5e-4)
